...
export function SwapWidget({
  ...
  // Fetch BSC quote whenever token changes
  ...
  return (
    <div style={embedded ? { background: '#fff' } : { border: `1.5px solid ${GOLD_DIM}`, borderRadius: 16, overflow: 'hidden', background: '#fff' }}>
      ...
      <div style={{ padding: '20px 20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {/* ── ETH chain: redirect to PancakeSwap cross-chain ── */}
        {isETH ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ background: '#F7F5F0', borderRadius: 12, padding: '14px 16px', fontSize: 13, color: TEXT, lineHeight: 1.6 }}>
              You&apos;re on <strong>Ethereum</strong>. PancakeSwap supports swapping USDT / USDC from ETH directly to $U on BNB Chain.
            </div>
            <a
            ...
            <p style={{ fontSize: 11, color: MUTED, margin: 0, lineHeight: 1.6 }}>
              Select USDT or USDC as input, set output to $U on BNB Chain. After swapping, switch your wallet to BNB Chain to continue here.
            </p>
          </div>
        ) : (
          <>
